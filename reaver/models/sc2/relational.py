...
def build_relational(obs_spec, act_spec, data_format='channels_first', broadcast_non_spatial=False, fc_dim=256):
    ...
    # TODO: obs_spec[2:] <- ['player', 'previous_action']
    # 'non-spatial': ['available_actions', 'player']
    non_spatial_inputs = [Input(s.shape) for s in obs_spec.spaces[2:]]
    non_spatial_inputs = Concatenate(axis=1, name='non_spatial_inputs')(non_spatial_inputs)
    ...
    channel_3 = 16
    output_3d = _deconv4x(relational_spatial, filters=[channel_3, channel_3],
    ...
    # CHECK THIS: logits[0] <- act#0: no_op
